...
public class ResourceLoader implements ImageObserver {
	...
	public AudioClip getSound(String name) {
		...
		try {
			...
			System.err.println("Cound not locate sound " + name + ": "
					+ e.getMessage());
		}
		return sound;
	}
	...
	/**
	 * creates a compatible image in memory, faster than using the original
	 * image format
	 * 
	 * @param width
	 *            image width
	...
	public BufferedImage getSprite(String name) {
		...
		try {
			...
			BufferedImage compatible = createCompatible(image.getWidth(),
					image.getHeight(), Transparency.BITMASK);
			compatible.getGraphics().drawImage(image, 0, 0, this);
			images.put(name, compatible);
		} catch (Exception e) {
			System.err.println("Cound not locate image " + name + ": "
					+ e.getMessage());
		}
		return image;
	}
