...
/**
 * @author Andrew Coleman Provides a Java implementation for a deterministic
 *         finite automata with the ability to minimize a machine.
 */
public class DFA {
	...
	/*
	 * format of the input file: IntegerN - the number of terminal characters in
	 * the machine Terminal * N - all of the terminals separated by a space
	 * NumStates - number of states in the machine - next is NumStates *
	 * IntegerN rows of transitions StartState TerminalTransitionChar EndState
	 * NumFinalStates - number of final states FinalStates - the final states of
	 * the machine separated by spaces
	 * 
	...
	public void minimize() {
		...
		for (int state = 0; state < numstates; state++) {
			if (minstates[state] == -1 || visited[state])
				continue;
			for (int p = 0; p < numstates; p++) {
				if (p == state || minstates[p] == -1)
					continue;
		...
		// rearrange all the paths using the minstate array into a new path
		// array
		int newpath[][] = new int[numstates][numterminals];
		boolean newfinal[] = new boolean[numstates];
		int minstatecounter = 0, minfinalcounter = 0;
		for (int state = 0; state < numstates; state++) {
			// find all accessible vertecies and vertecies that have not been
			// counted already
