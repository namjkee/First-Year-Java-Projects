...
public class ArrayPriorityQueue implements PriorityQueue {
	...
	public void decreaseKeys(Integer oldKey, Integer newKey) {
		// find the old key
		PriorityQueueNode nodetoModify = null;
		for (PriorityQueueNode n: queue) {
			if (n.key.equals(oldKey)) {
				nodetoModify = n;
				break;
			}
		}
		...
		queue.remove(nodetoModify);
		
		nodetoModify.key = newKey;
		queue.add(nodetoModify);
