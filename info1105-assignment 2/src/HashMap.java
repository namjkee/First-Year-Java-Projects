...
public class HashMap<K, V> {
	...
	public HashMap(int size, int multiplier, int modulus) {
...
//		putCollisions += 1;											// a putCollision
		while (hashMap[currentIndex] != null) {
			...
			currentIndex = (currentIndex + 1) % hashMap.length;		// index always wrap arounds incase
			probingSteps += 1;
			if (probingSteps == 1) putCollisions += 1;
	...
	public V get(K key) {
		...
		int currentIndex = (index + 1) % hashMap.length;
		...
		while (currentIndex != index) {
			currentIndex = (currentIndex + 1) % hashMap.length;							// wrapped index
			
			node = hashMap[currentIndex];						
			if (node != null && node.getKey() != null && node.getKey().equals(key)) {	// if key found - return it's value
				return (V) node.getValue();
			}
		}
